...
def printCPUUtil(rp_pilot_time_in_min):
    for i in range(rp_pilot_time_in_min * 12):
        ...
        time.sleep(4)
#parse collectl command for Polaris 

def parse_collectl(rp_pilot_time_in_min):
    for i in range(rp_pilot_time_in_min * 12):
        result = sp.run(['collectl', '-i5', '-c1'], stdout=sp.PIPE)
        output = result.stdout.decode('utf-8')
        ...
        for output in output_list:
            ...
            for line in lines:
                ...
                if len(words)>0 and words[0].isnumeric():
                    netin_bytes = int(words[8]) * 1024
                    netout_bytes = int(words[10]) *1024
                    print("NETWORK:", time.time(), netin_bytes, netout_bytes, flush=True)
...
def parse_iostat(rp_pilot_time_in_min):
    for i in range(rp_pilot_time_in_min*12):
        ...
        while index < len(line_list):
            ...
            index += 1 
        print("DISK:", time.time(), read_bytes, written_bytes, flush=True)
...
if __name__ == '__main__':
    ...
    t2 = threading.Thread(target=parse_collectl, args=(runtime_mins,))
