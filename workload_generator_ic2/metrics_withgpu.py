...
def add_metrics(task, node_list, num_all_nodes, pilot_id):
    ...
    for i in range(num_all_nodes):
       ...
       if node_name.strip().split('.')[0] in node_list:
    ...
    for watcher_id in watcher_ids: #NOT THE WATCHER ID - REWRITE 
        ...
        if len(lines)>0 and lines[0].strip().split('.')[0] in node_list:
            cpu_list=[]
            disk_list=[]
            network_list=[]
            memory_list=[]
            ...
            node_name = lines[0].strip().split('.')[0]
            dict= {}
    ...
    for taskid in task_list:
        ...
        for node_n in node_list:
            if node_n == dict["node_name"]:
                ...
                break
    f.close()
    print("TASK IDS ON NODE:", task_ids_on_node)
