...
def display_model(model_arch, model_weights, testX, testy, train_min, train_max):
    ...
    plt.ylim(-5,50)
    plt.scatter(testy[:,0],y_hat[:,0], facecolors='none', edgecolors='b')
    plt.xlabel("Actual GPU Utilization (%)")
    plt.ylabel("Predicted GPU Utilization (%)")
    plt.title("GPU Utilization - MLP Model - Polaris->IC2")
    plt.plot([0, 50], [0, 50], color = 'red', linewidth = 1)
    #plt.show()
