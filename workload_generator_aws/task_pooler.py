...
max_cores = 31 # 1 core per node is used for service tasks 
task_pool = []
def nas_pooler():
  cf = config.config
  if "nas" in cf.keys():
              ...
              for app in nas.omp_apps:
                  ...
                  while i <=rank_max:
                      for cl in cf["nas"]["class"]:
                        ...
                        if cl == "D" and app == "ft": #make sure ft.D.x does not run 
                            continue
                        if cl == "D":
              ...
              for app in nas.hybrid_apps:
                  ...
                  while i <=rank_max:
                    for cl in cf["nas"]["class"]:
                        ...
                        tdict = {"app": app+"."+cl+".x", 
                                 "desc":nas.hybrid_task_desc(app_path, i, max_cores)}
                        if cl == "D" and (app == "ft" or app == "is"): #make sure ft.D.x does not run 
                            continue
            ...
            for ds in cf["image_classification"]['data_size']:
                for ep in cf["image_classification"]['epochs']:
                    fdict = {"app": keras_app.keras_app_name+ "_bs"+str(bs) + "_ds"+ds+ "_ep"+str(ep),
                             "desc": keras_app.keras_app_task_desc(app_path, [keras_app.KERAS_DATA_PATH,bs, ds,ep,runtime_mins],max_cores)}

                    print(keras_app.keras_app_name+ "_bs"+str(bs) + "_ds"+ds+ "_ep"+str(ep), max_cores)
