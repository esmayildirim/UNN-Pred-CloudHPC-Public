...
def init(num_nodes, runtime_mins, service_path, venv_path):
    ...
    PILOT_DESCRIPTION = {
    'resource': 'bnl.aws',  # OR 'bnl.ic2_debug'
#    'project': 'csihpc',
    'nodes': num_nodes,
...
def submit_tasks(session, tmgr, task_desc_list, num_tasks,inter_arrival_times, seed_num): # multiple sessions must be possible
    ...
    for i in range(num_tasks):
        ...
        dict_task = metrics_withgpu.create_task(dict_task,
                                                ...
                                                (task.description['gpus_per_rank'], 0)['gpus_per_rank' in task.description.keys()])  
...
def write_workload_data(task_list, workload_name, platform, pilot_id, num_nodes, output_file): 
    ...
    for task_dict in task_list:
        ...
        print(float(task_dict['start_time']), float(task_dict['finish_time']))
...
if __name__ == "__main__":
    seed_list = [35, 62, 89, 22, 1]
    seed = 1
    allocated_num_nodes = 4
    allocated_time_mins = 1080
    num_tasks_to_submit = 50
    mean_inter_arrival_time_secs = 10
    ...
    session, pilot, tmgr = init(allocated_num_nodes,
                                allocated_time_mins,
                                "/home/ec2-user/workload_generator/",
                                "/home/ec2-user/ve.keras")
    ...
    write_workload_data(task_json_list,
                        "threaded-tasks"+str(seed),
                        "cloud_aws",
                        pilot.uid,
                        allocated_num_nodes, 
                        "workloads_threaded.json")
    ...
    write_system_load_data(session.uid,
                           ...
                           "threaded-tasks"+str(seed),
                           "cloud_aws",
                           "system_load_threaded.json")
    print("Finished with ", seed)
