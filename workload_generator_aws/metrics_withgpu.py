...
def add_start_finish(task, pilot_id):
    ...
    for line in lines:
        if 'rank_start' in line:
            stimestamp = line.split(',')[0].replace('\x00','')
            break
    for line in lines:
        if 'rank_stop' in line:
            etimestamp = line.split(',')[0].replace('\x00','')
        #print(stimestamp + '\t'+etimestamp)
    print(float(stimestamp), float(etimestamp))
        ...
        if len(lines)>0 and lines[0].strip() in node_list:
            ...
            for i in range(1, len(lines)):
                ...
                if lines[i].startswith("MEMORY:"):
                    ...
                    if float(memory_metrics[1].strip()) >= float(task["start_time"]) and float(memory_metrics[1].strip()) <= float(task["finish_time"]):
